...
class OpenAIClient(BaseLLMModel):
    ...
    def get_answer_stream_iter(self):
        ...
        if 'moon' in self.model_name:
            response = self.moonshot_responce(stream=True)
        else:
    ...
    def moonshot_responce(self, stream=False):
        ...
        client = OpenAI(
            ...
            base_url=shared.state.moonshot_chat_completion_url,
            timeout=timeout,
        )
        response = client.chat.completions.create(
            ...
            max_tokens = self.max_generation_token if self.max_generation_token is not None else None,
            stop = self.stop_sequence if self.stop_sequence is not None else None,
            logit_bias = self.encoded_logit_bias() if self.logit_bias is not None else None,
            user  = self.user_identifier if self.user_identifier else None,
        )
        return response
    @shared.state.switching_api_key  # 在不开启多账号模式的时候，这个装饰器不会起作用
        ...
        try:
            ...
            for idx, chunk in enumerate(response):
                ...
                if not chunk_message.content:
                    continue
                yield chunk_message.content  # save the message
